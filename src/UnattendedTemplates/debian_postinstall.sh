...
MY_EXITCODE=0
MY_DEBUG="yes" # "yes"
MY_XKBLAYOUT="$(echo @@VBOX_INSERT_LOCALE@@ | awk -F_ '{print $1}')"
...
#
# Custom entries vbox-cli
#
echo "--------------------------------------------------" >>"${MY_LOGFILE}"
echo '** Configuring enviroment according user configuration...' | tee -a "${MY_LOGFILE}"
log_command_in_target sed "/root/ i @@VBOX_INSERT_USER_LOGIN@@    ALL=(ALL:ALL) ALL" -i /etc/sudoers
log_command_in_target sed -i "s/XKBLAYOUT=\"[a-z]+\"/XKBLAYOUT=\"$MY_XKBLAYOUT\"/" /etc/default/keyboard
log_command_in_target gsettings reset org.gnome.desktop.input-sources xkb-options
